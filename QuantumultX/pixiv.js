...
switch (path) {
    ...
    case '/touch/ajax/user/self/status':
        body = body.boby['user_status'];
        body['ads_disabled'] = true;
        body['show_ads'] = false;
        body['user_premium'] = 1;
        break;
    ...
    return new (class {
        ...
        initCache() {
            if (
                ...
                (o || n) && (this.cache = JSON.parse($persistentStore.read(this.name) || '{}')),
                i)
            ) {
                let e = 'root.json';
        ...
        persistCache() {
            ...
            s && $prefs.setValueForKey(e, this.name),
                ...
                i &&
                    (this.node.fs.writeFileSync(`${this.name}.json`, e, { flag: 'w' }, (e) =>
                        console.log(e)
                    ),
